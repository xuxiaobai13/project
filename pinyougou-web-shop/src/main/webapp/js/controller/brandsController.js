//控制层
app.controller('brandsController' ,function($scope,$controller,$location,typeTemplateService ,itemCatService,uploadService ,brandsService){

    $controller('baseController',{$scope:$scope});//继承

    //读取列表数据绑定到表单中
    $scope.findAll=function(){
        brandsService.findAll().success(
            function(response){
                $scope.list=response;
            }
        );
    ...
    $scope.findPage=function(page,rows){
        brandsService.findPage(page,rows).success(
            function(response){
    ...
    $scope.findOne=function(){
        ...
        //点击修改按钮 过来的 应该回显
        brandsService.findOne(id).success(// 通过商品ID 查询 商品包装对象（商品对象 商品详情对象 库存结果集对象）
            function(response){
    ...
    $scope.save=function(){
        ...
        if($scope.entity.goods.id!=null){//如果有ID
            serviceObject=brandsService.update( $scope.entity ); //修改
        }else{
            serviceObject=brandsService.add( $scope.entity  );//增加
        }
    ...
    $scope.dele=function(){
        //获取选中的复选框
        brandsService.dele( $scope.selectIds ).success(
            function(response){
                if(response.flag){
    ...
    $scope.search=function(page,rows){
        brandsService.search(page,rows,$scope.searchEntity).success(
            function(response){
                $scope.list=response.rows;
                $scope.paginationConf.totalItems=response.total;//更新总记录数
            }
